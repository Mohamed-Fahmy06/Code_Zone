            ...
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6 text-start">
              ...
              <div className="space-y-2">
                ...
                <select className="glow-input w-full bg-surface-container-low border border-white/10 focus:ring-0 rounded-xl p-3 md:p-4 text-on-surface outline-none transition-all appearance-none cursor-pointer text-start">
                  ...
                  <option>{language === 'ar' ? 'خدمات الذكاء الاصطناعي' : 'AI Services'}</option>
              ...
              >
                {status === 'idle' && (language === 'ar' ? 'إرسال الرسالة' : 'Send Message')}
                {status === 'sending' && (language === 'ar' ? 'جاري الإرسال...' : 'Sending...')}
                {status === 'success' && (language === 'ar' ? 'تم الإرسال بنجاح!' : 'Sent Successfully!')}
              </button>
            </form>
          </div>
            ...
            <div className="glass-panel rounded-2xl p-lg relative overflow-hidden border border-white/5">
              ...
              <div className="space-y-6 md:space-y-8">
                <div className="flex items-center gap-4 group">
                  ...
                  <div>
                    <p className="font-code-label text-[10px] md:text-code-label text-secondary">{language === 'ar' ? 'الهاتف' : 'Phone'}</p>
                ...
                <div className="flex items-center gap-4 group">
                  ...
                  <div>
                    <p className="font-code-label text-[10px] md:text-code-label text-secondary">{language === 'ar' ? 'البريد الإلكتروني' : 'Email'}</p>
                ...
                <div className="flex items-center gap-4 group">
                  ...
                  <div>
                    <p className="font-code-label text-[10px] md:text-code-label text-secondary">{language === 'ar' ? 'الموقع' : 'Location'}</p>
                    <p className="font-title-md text-base md:text-title-md text-on-surface">{language === 'ar' ? 'الرياض، مجمع الابتكار التقني' : 'Riyadh, Tech Innovation Hub'}</p>
                  </div>
                </div>
              </div>
